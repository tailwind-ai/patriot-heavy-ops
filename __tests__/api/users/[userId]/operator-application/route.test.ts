  ...
  describe("POST /api/users/[userId]/operator-application", () => {
    describe("Authentication & Authorization", () => {
      it("should return 403 when no session exists", async () => {
        ...
        const request = createMockRequest(
          "POST",
          `http://localhost:3000/api/users/${mockUserId}/operator-application`,
          applicationData
        )
    ...
    describe("Data Validation", () => {
      ...
      it("should require location field", async () => {
        ...
        const request = createMockRequest(
          "POST",
          "http://localhost:3000/api/users/test/operator-application",
          invalidData
        )
        const response = await POST(request, mockContext)
        ...
        expect(
          errors.some(
            (error: { message?: string }) =>
              error.message === "Please select a location"
          )
        ).toBe(true)
      })
    ...
    describe("Route Context Validation", () => {
      ...
      it("should handle empty userId parameter", async () => {
        ...
        // Create context with empty userId using Promise structure (Next.js 15)
        const invalidContext = { params: Promise.resolve({ userId: "" }) }
