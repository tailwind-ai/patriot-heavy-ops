...
describe('Utility Functions', () => {
  describe('cn function (className merger)', () => {
    ...
    it('should handle conditional classes', () => {
      ...
      expect(cn('base', undefined && 'conditional')).toBe('base')
    })
    ...
    it('should preserve non-conflicting classes', () => {
      expect(cn('bg-red-500', 'text-white', 'rounded-lg', 'shadow-md'))
        .toBe('bg-red-500 text-white rounded-lg shadow-md')
    })
  ...
  describe('absoluteUrl function', () => {
    ...
    it('should handle paths with query parameters', () => {
      expect(absoluteUrl('/search?q=test')).toBe('https://example.com/search?q=test')
      expect(absoluteUrl('/users?page=1&limit=10')).toBe('https://example.com/users?page=1&limit=10')
    })

    it('should handle paths with fragments', () => {
      expect(absoluteUrl('/docs#introduction')).toBe('https://example.com/docs#introduction')
      expect(absoluteUrl('/page#section-1')).toBe('https://example.com/page#section-1')
    })

    it('should handle complex paths', () => {
      expect(absoluteUrl('/api/v1/users/123/posts?published=true#comments'))
        .toBe('https://example.com/api/v1/users/123/posts?published=true#comments')
    })

    it('should handle special characters in paths', () => {
      expect(absoluteUrl('/search?q=hello%20world')).toBe('https://example.com/search?q=hello%20world')
      expect(absoluteUrl('/files/document%20name.pdf')).toBe('https://example.com/files/document%20name.pdf')
    })

    it('should not double-encode already encoded paths', () => {
      expect(absoluteUrl('/search?q=hello+world')).toBe('https://example.com/search?q=hello+world')
      expect(absoluteUrl('/path/with%20spaces')).toBe('https://example.com/path/with%20spaces')
    })
  })
