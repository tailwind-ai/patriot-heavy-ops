...
describe('AnaAnalyzer - Vercel Deployment Analysis', () => {
  ...
  describe('analyzeVercelDeploymentLogs', () => {
    it('should parse build timeout errors correctly', () => {
      ...
      const result = analyzer.analyzeVercelDeploymentLogs('Vercel Deploy', logContent)
      ...
      expect(result.issues[0]).toMatchObject({
        ...
        suggestedFix: expected.suggestedFixes[0],
      })
    })
    ...
    it('should parse dependency installation failures correctly', () => {
      ...
      const result = analyzer.analyzeVercelDeploymentLogs('Vercel Deploy', logContent)
      const expected = getExpectedVercelResults('DEPENDENCY_INSTALLATION_FAILED')
    ...
    it('should parse TypeScript build errors correctly', () => {
      ...
      expect(result.issues[0]).toMatchObject({
        ...
        priority: expected.priorities[0],
        files: expected.files ? [expected.files[0]] : undefined,
        lineNumbers: expected.lineNumbers ? [expected.lineNumbers[0]] : undefined,
        rootCause: expected.rootCauses[0],
      })
    })
    ...
    it('should handle mixed Vercel errors correctly', () => {
      ...
      expect(result.issues).toHaveLength(expected.issueCount)
      expect(result.issues.map(issue => issue.priority)).toEqual(expected.priorities)
      expect(result.issues.map(issue => issue.rootCause)).toEqual(expected.rootCauses)
    })
  })
  ...
  describe('Vercel Error Pattern Recognition', () => {
    ...
    it('should recognize environment variable patterns', () => {
      const logContent = 'Environment variable DATABASE_URL is required but not set'
    ...
    it('should recognize function size limit patterns', () => {
      const logContent = 'Serverless Function "api/large.js" exceeds the maximum size limit'
      expect(VERCEL_ERROR_PATTERNS.FUNCTION_SIZE_LIMIT.test(logContent)).toBe(true)
    })

    it('should recognize static generation patterns', () => {
      const logContent = 'Error occurred prerendering page "/products/[id]"'
      expect(VERCEL_ERROR_PATTERNS.STATIC_GENERATION.test(logContent)).toBe(true)
    })
  ...
  describe('Vercel Priority Calculation', () => {
    it('should assign critical priority to build timeouts', () => {
      const priority = analyzer.calculateVercelPriority('build timed out', 'Vercel Deploy')
      expect(priority).toBe('critical')
    })

    it('should assign critical priority to memory limit exceeded', () => {
      const priority = analyzer.calculateVercelPriority('heap out of memory', 'Vercel Deploy')
      expect(priority).toBe('critical')
    })

    it('should assign high priority to environment variable issues', () => {
      const priority = analyzer.calculateVercelPriority('environment variable required', 'Vercel Deploy')
      expect(priority).toBe('high')
    })

    it('should assign high priority to dependency issues', () => {
      const priority = analyzer.calculateVercelPriority('npm ERR!', 'Vercel Deploy')
      expect(priority).toBe('high')
    })

    it('should assign high priority to function size limits', () => {
      const priority = analyzer.calculateVercelPriority('exceeds the maximum size limit', 'Vercel Deploy')
      expect(priority).toBe('high')
    })

    it('should assign medium priority to unknown Vercel errors', () => {
      const priority = analyzer.calculateVercelPriority('unknown vercel error', 'Vercel Deploy')
  ...
  describe('Vercel Suggested Fix Generation', () => {
    it('should generate specific fix for build timeouts', () => {
      const fix = analyzer.generateVercelSuggestedFix('build timed out after 10 minutes')
      expect(fix).toBe('Optimize build process to complete within time limit, consider build caching')
    })

    it('should generate specific fix for memory issues', () => {
      const fix = analyzer.generateVercelSuggestedFix('JavaScript heap out of memory')
      expect(fix).toBe('Reduce memory usage during build, optimize large dependencies')
    })

    it('should generate specific fix for environment variables', () => {
      const fix = analyzer.generateVercelSuggestedFix('Environment variable DATABASE_URL is required')
      expect(fix).toBe('Add DATABASE_URL environment variable in Vercel dashboard')
    })

    it('should generate specific fix for dependency issues', () => {
      const fix = analyzer.generateVercelSuggestedFix('npm ERR! ERESOLVE unable to resolve')
    ...
    it('should generate specific fix for function size limits', () => {
      const fix = analyzer.generateVercelSuggestedFix('exceeds the maximum size limit of 50 MB')
    ...
    it('should generate specific fix for static generation errors', () => {
      const fix = analyzer.generateVercelSuggestedFix('Error occurred prerendering page "/products/[id]"')
      expect(fix).toBe('Fix getStaticProps error in /products/[id] page')
    })

    it('should generate specific fix for Edge Runtime errors', () => {
      const fix = analyzer.generateVercelSuggestedFix('Edge Runtime does not support Node.js')
      expect(fix).toBe('Replace Node.js modules with Edge Runtime compatible alternatives')
    })

    it('should generate generic fix for unknown errors', () => {
      const fix = analyzer.generateVercelSuggestedFix('unknown deployment error')
      expect(fix).toBe('Review Vercel deployment logs for specific error details')
    })
  })

  describe('Vercel Root Cause Extraction', () => {
    it('should identify build timeout root cause', () => {
      const rootCause = analyzer.extractVercelRootCause('build timed out after 10 minutes')
      expect(rootCause).toBe('Vercel build timeout')
    })

    it('should identify memory limit root cause', () => {
      const rootCause = analyzer.extractVercelRootCause('JavaScript heap out of memory')
      expect(rootCause).toBe('Memory limit exceeded')
    })

    it('should identify environment variable root cause', () => {
      const rootCause = analyzer.extractVercelRootCause('Environment variable DATABASE_URL is required')
      expect(rootCause).toBe('Missing environment variable')
    })

    it('should identify dependency resolution root cause', () => {
      const rootCause = analyzer.extractVercelRootCause('npm ERR! ERESOLVE unable to resolve')
      expect(rootCause).toBe('Dependency resolution conflict')
    })

    it('should identify function size limit root cause', () => {
      const rootCause = analyzer.extractVercelRootCause('exceeds the maximum size limit')
      expect(rootCause).toBe('Serverless function size limit exceeded')
    })

    it('should identify static generation root cause', () => {
      const rootCause = analyzer.extractVercelRootCause('Error occurred prerendering page')
      expect(rootCause).toBe('Static page generation failure')
    })

    it('should identify Edge Runtime root cause', () => {
      const rootCause = analyzer.extractVercelRootCause('Edge Runtime does not support')
      expect(rootCause).toBe('Edge Runtime compatibility issue')
    })

    it('should handle unknown Vercel errors', () => {
      const rootCause = analyzer.extractVercelRootCause('unknown deployment error')
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle empty Vercel logs', () => {
      ...
      expect(result.issues[0].rootCause).toBe('Unknown Vercel deployment failure')
    })
    ...
    it('should handle malformed Vercel logs', () => {
      ...
      const result = analyzer.analyzeVercelDeploymentLogs('Vercel Deploy', logContent)
      expect(result.issues).toHaveLength(1)
      expect(result.issues[0].description).toContain('failed - check logs for details')
    })
    ...
    it('should handle very large Vercel logs efficiently', () => {
      ...
      const result = analyzer.analyzeVercelDeploymentLogs('Vercel Deploy', largeLogContent)
