...
export class ServiceRequestService extends BaseService {
  ...
  private static readonly VALID_STATUS_TRANSITIONS = new Map([
    ...
    ['OPERATOR_ASSIGNED', ['EQUIPMENT_CHECKING', 'OPERATOR_MATCHING', 'CANCELLED']],
    ['EQUIPMENT_CHECKING', ['EQUIPMENT_CONFIRMED', 'OPERATOR_MATCHING', 'CANCELLED']],
   ...
   */
  public calculateTotalHours(durationType: DurationType, durationValue: number): ServiceResult<number> {
    ...
    if (!validation.success) {
      return validation as ServiceResult<number>;
    }

    if (durationValue <= 0) {
      return this.createError('INVALID_DURATION', 'Duration value must be positive', {
        durationValue,
      });
    }
    ...
    switch (durationType) {
      ...
      case 'WEEKLY':
        ...
        break;
      default:
        return this.createError('INVALID_DURATION_TYPE', 'Invalid duration type', {
          durationType,
        });
    }
   ...
   */
  public getDurationDisplayText(durationType: DurationType, durationValue: number): ServiceResult<string> {
   ...
   */
  public calculateTransportFee(transport: TransportOption): ServiceResult<number> {
    ...
    if (fee === undefined) {
      return this.createError('INVALID_TRANSPORT_OPTION', 'Invalid transport option', {
        transport,
      });
    }
  ...
  public calculateBaseCost(
    ...
    this.logOperation('calculateBaseCost', { totalHours, baseRate, rateType, equipmentCategory });
    ...
    if (totalHours <= 0 || baseRate <= 0) {
      return this.createError('INVALID_CALCULATION_INPUT', 'Hours and rate must be positive', {
        totalHours,
        baseRate,
      });
    }
    ...
    if (multiplier === undefined) {
      return this.createError('INVALID_EQUIPMENT_CATEGORY', 'Invalid equipment category', {
        equipmentCategory,
      });
    }
   ...
   */
  public calculateServiceRequestPricing(input: ServiceRequestCalculationInput): ServiceResult<ServiceRequestCalculationResult> {
    this.logOperation('calculateServiceRequestPricing', input);

    // Calculate total hours
    const hoursResult = this.calculateTotalHours(input.durationType, input.durationValue);
    if (!hoursResult.success) {
      return hoursResult as ServiceResult<ServiceRequestCalculationResult>;
    }

    // Calculate duration display
    const displayResult = this.getDurationDisplayText(input.durationType, input.durationValue);
    if (!displayResult.success) {
      return displayResult as ServiceResult<ServiceRequestCalculationResult>;
    }
    ...
    if (!baseCostResult.success) {
      return baseCostResult as ServiceResult<ServiceRequestCalculationResult>;
    }

    // Calculate transport fee
    const transportResult = this.calculateTransportFee(input.transport);
    if (!transportResult.success) {
      return transportResult as ServiceResult<ServiceRequestCalculationResult>;
    }
   ...
   */
  public validateStatusTransition(fromStatus: string | undefined, toStatus: string): ServiceResult<StatusTransition> {
    this.logOperation('validateStatusTransition', { fromStatus, toStatus });

    const validation = this.validateRequired({ toStatus }, ['toStatus']);
    if (!validation.success) {
      return validation as ServiceResult<StatusTransition>;
    }
    ...
    const validTransitions = ServiceRequestService.VALID_STATUS_TRANSITIONS.get(fromStatus);
    if (!validTransitions) {
      return this.createSuccess({
        fromStatus,
        toStatus,
  ...
  public getValidNextStatuses(currentStatus: string): ServiceResult<string[]> {
    ...
    const validation = this.validateRequired({ currentStatus }, ['currentStatus']);
    if (!validation.success) {
      return validation as ServiceResult<string[]>;
    }

    const validStatuses = ServiceRequestService.VALID_STATUS_TRANSITIONS.get(currentStatus);
    if (!validStatuses) {
  ...
  public validateServiceRequestBusinessRules(data: {
    ...
    if (data.durationValue > 30 && (data.durationType === 'HALF_DAY' || data.durationType === 'FULL_DAY')) {
