...
export const AnalyzedFailureSchema = z.object({
  ...
  type: z.enum(['ci_failure', 'vercel_failure'], {
    errorMap: () => ({ message: 'Type must be either ci_failure or vercel_failure' }),
  }),
...
export function createAnalyzedFailure(
  data: Omit<AnalyzedFailure, 'id' | 'createdAt'> & { id?: string; createdAt?: string }
): AnalyzedFailure {
  const now = new Date().toISOString()
  const id = data.id || `${data.type.replace('_', '-')}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
...
export const ERROR_PATTERNS = {
  TYPESCRIPT: {
    regex: /(?:error\s+in\s+([^\s]+\.(ts|tsx)):(\d+):(\d+)|([^\s]+\.(ts|tsx)):(\d+):(\d+)\s+-\s+error\s+TS\d+)/gi,
...
export const VERCEL_ERROR_PATTERNS = {
  ...
  MEMORY_LIMIT: {
    regex: /(?:javascript heap out of memory|process ran out of memory)(?![\s\S]*(?:javascript heap out of memory|process ran out of memory))/gi,
    priority: 'critical' as const,
  ...
  FUNCTION_SIZE_LIMIT: {
    regex: /serverless function .+ exceeds the (?:maximum )?size limit|serverless function .+ exceeds the limit/gi,
    priority: 'high' as const,
  ...
  DATABASE_CONNECTION: {
    regex: /(?:can't reach database server|prisma schema validation failed)(?![\s\S]*(?:can't reach database server|prisma schema validation failed))/gi,
    priority: 'high' as const,
...
export function isFailureType(value: unknown): value is FailureType {
  return typeof value === 'string' && ['ci_failure', 'vercel_failure'].includes(value)
}
